...
public class Lanes {
    ...
    public Lanes(BaseLayoutManager layout, UserContainerLayoutManager.Orientation orientation, Rect[] lanes, int laneSize) {
        mLayout = layout;
        mIsVertical = (orientation == UserContainerLayoutManager.Orientation.VERTICAL);
        mLanes = lanes;
        mLaneSize = laneSize;
    ...
    public UserContainerLayoutManager.Orientation getOrientation() {
        return (mIsVertical ? UserContainerLayoutManager.Orientation.VERTICAL : UserContainerLayoutManager.Orientation.HORIZONTAL);
    }
    ...
    public int pushChildFrame(Rect outRect, int lane, int margin, UserContainerLayoutManager.Direction direction) {
        final int delta;

        final Rect laneRect = mLanes[lane];
        if (mIsVertical) {
            if (direction == UserContainerLayoutManager.Direction.END) {
    ...
    public void popChildFrame(Rect outRect, int lane, int margin, UserContainerLayoutManager.Direction direction) {
        final Rect laneRect = mLanes[lane];
        if (mIsVertical) {
            if (direction == UserContainerLayoutManager.Direction.END) {
    ...
    public void getChildFrame(Rect outRect, int childWidth, int childHeight, LaneInfo laneInfo,
                              UserContainerLayoutManager.Direction direction) {
        ...
        final int anchorLane =
                (direction == UserContainerLayoutManager.Direction.END ? laneInfo.anchorLane : laneInfo.startLane);
        final Rect anchorRect = mLanes[anchorLane];

        if (mIsVertical) {
            outRect.left = startRect.left;
            outRect.top =
                    (direction == UserContainerLayoutManager.Direction.END ? anchorRect.bottom : anchorRect.top - childHeight);
        } else {
            outRect.top = startRect.top;
            outRect.left =
                    (direction == UserContainerLayoutManager.Direction.END ? anchorRect.right : anchorRect.left - childWidth);
        }
    ...
    private int findLaneThatFitsSpan(int anchorLane, int laneSpan, UserContainerLayoutManager.Direction direction) {
    ...
    public void findLane(LaneInfo outInfo, int laneSpan, UserContainerLayoutManager.Direction direction) {
        outInfo.setUndefined();

        int targetEdge = (direction == UserContainerLayoutManager.Direction.END ? Integer.MAX_VALUE : Integer.MIN_VALUE);
        for (int l = 0; l < mLanes.length; l++) {
            final int laneEdge;
            if (mIsVertical) {
                laneEdge = (direction == UserContainerLayoutManager.Direction.END ? mLanes[l].bottom : mLanes[l].top);
            } else {
                laneEdge = (direction == UserContainerLayoutManager.Direction.END ? mLanes[l].right : mLanes[l].left);
            }

            if ((direction == UserContainerLayoutManager.Direction.END && laneEdge < targetEdge) ||
                    (direction == UserContainerLayoutManager.Direction.START && laneEdge > targetEdge)) {
    ...
    public void reset(UserContainerLayoutManager.Direction direction) {
        for (int i = 0; i < mLanes.length; i++) {
            final Rect laneRect = mLanes[i];
            if (mIsVertical) {
                if (direction == UserContainerLayoutManager.Direction.START) {
